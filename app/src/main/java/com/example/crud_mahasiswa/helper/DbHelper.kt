package com.example.crud_mahasiswa.helper
...
class DbHelper(context: Context) :
    ...
    fun getAllDataMahasiswa() : List<ModelMahasiswa>{
        ...
        while (cursor.moveToNext()){
            ...
            Log.d("Databasehelper","Fechted ID : $id, namamahasiswa: $nama_mahasiswa")
    ...
    fun getMhsById(mhsId : Int): ModelMahasiswa{
        val db = readableDatabase
        val query = "SELECT * FORM $TABLE_NAME WHERE $ID_COL = $mhsId"
        val cursor = db.rawQuery(query, null)
        cursor.moveToNext()
